...
25. **Pomodoro timer durations not refreshing after settings change** — SettingsPage and PomodoroPage each have separate `PomodoroViewModel` instances; saving config in Settings only persisted to disk but PomodoroPage's VM never reloaded it → added `NavDestination.onShown()` on PomodoroPage that calls `refreshConfig()` + new `PomodoroViewModel.reloadConfig()` (loads from disk, resyncs durations when idle).
26. **Pomodoro start/pause button state not updating** — `@State displayState` updated from `setInterval` callback (via `tickHandler` closure) may not reliably trigger ArkUI re-render across component boundaries → added `@State stateUpdateSignal: number` counter incremented in `tickHandler` alongside `displayState` update, guaranteeing build() re-executes and `@Prop currentState` propagates to `PomodoroControls`.
27. **Pomodoro circle uses multi-dash rotating animation** — `strokeDashArray([progress*100, 100-progress*100])` produced repeating dash/gap pattern resembling multiple rotating dots → rewrote `PomodoroCircle` with circumference-based single-arc formula `strokeDashArray([C*progress, C*(1-progress)])` for gapless single arc, `strokeDashOffset(-C/4)` so the arc starts at 12-o'clock, and `strokeLineCap(Round)` for a smooth leading dot that grows to fill the ring.
