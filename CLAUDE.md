...
61. **专辑打开后无歌曲 + 无法流式播放 + MusicPage 重构为纯 Navidrome 流媒体（5 轮修复）** — 五个根因：⓪ **`getAlbumList2` JSON key 错误（第二轮新发现）** — Subsonic API 中 `getAlbumList` 返回 key `"albumList"`，`getAlbumList2` 返回 key `"albumList2"`，代码调 `getAlbumList2` 却读 `albumList` → 永远 undefined → 永远空数组，导致"专辑获取失败"；① **`getSongs()` 缺少 status 检查** — API 返回 HTTP 200 但 Subsonic status 可能不是 ok，代码直接读 `resp['album']['song']` 无错误处理；② **错误静默吞噬** — `browseNovidromeSongs/Albums/Artists` 的 catch 块设 `=[]` 丢弃异常，用户永远看不到失败原因；③ **AVPlayer 播放竞态条件** — `loadTrack()` 设 URL 后 AVPlayer 异步准备（idle→initialized→prepared），`play()` 立即检查状态只放行 prepared/paused/completed，HTTP 流媒体准备远慢于本地文件 → `play()` 静默无操作 → 永远不响；④ **MusicPage 架构错位 + 层级切换残影** — 没有本地音乐却保留本地曲库标签，Navidrome 是需手动进入的独立浏览模式；层级切换时 browseLevel 先于 isLoading 变更导致旧视图在新视图渲染前被销毁产生残影。修复：⓪ `SubsonicAlbumListResponse` 新增 `albumList2?:` 可选字段，`getAlbums()` 优先读 `albumList2` fallback `albumList`；`getArtist` 路径新增 status 检查；① `getSongs()` 新增 status 检查 + `encodeURIComponent(albumId)` + 防御单曲 object/array 兼容 + hilog 日志；② MusicViewModel 新增 `@Trace novidromeError` 字段；③ MusicPlayerService 新增 `autoPlayWhenReady` + `reset()` 替代 `stop()`；④ MusicPage 完全重写 — 移除本地标签/导入按钮；loading 遮罩在 browseLevel 变更之前先设为 true（`isLoading=true` → `browseLevel=N` → 加载数据 → `isLoading=false`），返回按钮用 `await Promise.resolve()` 强制 microtask yield；header 歌曲层显示返回+专辑名。
