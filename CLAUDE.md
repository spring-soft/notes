...
25. **Pomodoro timer durations not refreshing after settings change** (3 rounds) — Round 1: added `NavDestination.onShown()` + `PomodoroViewModel.reloadConfig()`. Round 2: card chips still showed stale numbers → added explicit `@State chipWorkSecs/chipShortSecs/chipLongSecs` synced via `syncChipDurations()`. Round 3: SettingsPage had a separate "保存番茄钟设置" button → removed it; added `onSliderChanged` callback + `schedulePomodoroSave()` (400ms debounce) + `persistConfig()` for immediate save-on-slide; now slider changes persist immediately and PomodoroPage reloads fresh config on `onShown`.
...
27. **Pomodoro circle uses multi-dash rotating animation** (3 rounds) — Round 1: rewrote with circumference-based formula. Round 2: switched from negative `strokeDashOffset` to `.rotate({ angle: -90 })`. Round 3: `strokeLineCap(Round)` caused two caps (start+end) → changed to `Butt` on the arc and added a separate third layer: a 12×12 filled Circle inside a 280×280 Column rotated by `progress*360`° to serve as the single leading dot; hidden when progress=0.
