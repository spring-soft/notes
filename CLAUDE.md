...
25. **Pomodoro timer durations not refreshing after settings change** (2 rounds) — Round 1: SettingsPage and PomodoroPage each have separate `PomodoroViewModel` instances; added `NavDestination.onShown()` + `PomodoroViewModel.reloadConfig()`. Round 2: card chips still showed stale numbers because `@State viewModel`'s nested `config.*` properties may not trigger ArkUI re-render reliably → added explicit `@State chipWorkSecs/chipShortSecs/chipLongSecs` fields synced via `syncChipDurations()` on init-complete and onShown; build() now reads @State fields directly.
26. **Pomodoro start/pause button state not updating** (2 rounds) — Round 1: added `@State stateUpdateSignal` counter. Round 2: `@Prop currentState` (enum) comparison across component boundaries from setInterval context may still be unreliable → added `@Prop isRunning: boolean` + `@Prop stateSignal: number` to `PomodoroControls`; parent passes `this.isTimerRunning()` (direct bool) and `this.stateUpdateSignal` (forces child re-render); removed old `private get isRunning()` to avoid duplicate identifier.
27. **Pomodoro circle uses multi-dash rotating animation** (2 rounds) — Round 1: rewrote `PomodoroCircle` with circumference-based formula `strokeDashArray([C*progress, C*(1-progress)])` and `strokeDashOffset(-C/4)` for 12-o'clock start. Round 2: negative `strokeDashOffset` not supported in ArkUI → switched to `.rotate({ angle: -90 })` (universal attribute, rotates circle so dash starts at top), kept `StrokeLineCap.Round` for smooth leading dot.
28. **Custom background image picker not working** — `DocumentViewPicker` (designed for documents) could not properly access image files in HarmonyOS NEXT → switched to `photoAccessHelper.PhotoViewPicker` (HarmonyOS-recommended photo picker API, `@ohos.file.photoAccessHelper` default import), `PhotoViewMIMETypes.IMAGE_TYPE` filter, `PhotoSelectResult.photoUris[0]` for the returned image URI usable by `Image()`.
