...
55. **TodoRepository 显式事务移除 + onSave await 恢复** — 两轮修复：① 移除 TodoRepository 全部 6 个写操作的显式事务（`beginTransaction`/`commit`/`rollBack`），改为与 NoteRepository 完全一致的 plain `await this.store.insert/update/delete()`——NoteRepository 无事务能持久化，显式事务反而可能引入 WAL 刷新时序问题；② `onSave` 从 fire-and-forget 改为 `.then()` 链式模式——`@Event` 要求 `=> void` 返回类型，sync 回调内调用 `handleTodoSave().then(() => { showAddSheet = false })` 既满足类型约束又确保 DB 写入完成后才关闭 sheet 触发 onShown→loadData()，杜绝「保存后杀后台数据丢失」的竞态窗口；③ 新增 `isSaving` 状态防重复提交。
