...
| `service/MusicPlayerService.ets` | AVPlayer wrapper with state machine, callback pattern, waitForState() async guard |
...
64. **笔记详情页导航动画横向位移修复** — 根因：`Navigation` 默认 Stack 模式的 push/pop 动画是 Slide（从左滑入/向左滑出），导致笔记列表在 push 时 header 和卡片一起左滑才消失。修复：所有 `pushPathByName()` 和 `pop()` 调用传入第三个参数 `false` 禁用动画，页面原地切换无位移。涉及 `NotesPage.ets`、`NoteDetailPage.ets`、`MusicPage.ets`、`MusicPlayerPage.ets`、`Index.ets`。SDK 24 中第三参数类型为 `animated?: boolean`，不是对象。**（注：此修复在 #66 中被回退 — `false` 导致页面切换完全无动画过于生硬，恢复 `true` 使用默认平滑过渡。）**
...
66. **导航动画过于生硬 — 恢复平滑过渡** — 根因：Bug #64 为修复横向位移把全部 `pushPathByName()`/`pop()` 的动画参数设为 `false`，完全禁用了 Navigation 过渡动画，导致页面瞬间切换无任何过渡——从"有动画但方向不对"变成了"完全没有动画"。修复：全部 8 处导航调用（`NotesPage`、`NoteDetailPage` ×3、`MusicPage`、`MusicPlayerPage`、`Index` ×2）的第三个参数从 `false` 恢复为 `true`，使用 Navigation 默认 Stack 滑动过渡——这是标准移动端导航模式，提供平滑视觉连续性。
