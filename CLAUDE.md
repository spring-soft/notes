...
66. **导航动画过于生硬 — Fade 淡入淡出替代 Slide** — 三轮迭代：① #64 把全部 push/pop 动画参数设为 `false` 禁用 Slide → 页面瞬间切换无过渡，生硬。② 第一轮修复把 `false` 恢复到 `true` → 恢复了平滑过渡但也带回了 Slide 横向位移（老组件跟着一起运动）。③ **最终方案（本轮）**：保持 `pushPathByName`/`pop` 使用 `false`（禁用默认 Slide），在 NavDestination 页面手动实现 Fade 动画：`@Local pageOpacity` + `aboutToAppear()` 中 `animateTo` 驱动 0→1（入场）、`animatedPop()` 先设 1→0 再 `setTimeout` 260ms → `pop(false)`（退场）。外层 Stack 加 `.opacity(this.pageOpacity).animation({ duration: 250, curve: Curve.EaseInOut })`。效果：纯淡入淡出，无横向位移，平滑自然。
