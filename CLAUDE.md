...
59. **Navidrome 设置页无法连接** — 两个根因：① HarmonyOS NEXT 默认拦截 HTTP 明文流量（`cleartextTraffic` 未开启），Navidrome 本地服务器通常用 HTTP 非 HTTPS，所有连接静默失败；② `ping()` catch 块丢弃了 `BusinessError` 信息（`catch (_err) { return false; }`），用户只看到泛泛「✗ 连接失败」无任何具体原因。修复：① `module.json5` 新增 `"network": { "cleartextTraffic": true }` 允许 HTTP；② `NavidromeApiClient` 新增 `lastError` 字段，`ping()` 失败时记录错误消息；③ `MusicViewModel` 新增 `getNovidromeError()` 暴露错误；④ `SettingsPage.testNovidromeConnection()` 显示具体错误信息（如 "Cleartext HTTP traffic not permitted"）替代泛泛「连接失败」。
