...
60. **Navidrome 连接成功但音乐页无内容** — 根因：MusicPage 只展示本地 RDB 曲库（`viewModel.tracks`），没有 Navidrome 远程浏览/导入 UI。MusicViewModel 虽已实现 `browseNovidromeArtists/Albums/Songs`、`searchNovidrome`、`importNovidromeTrack` 等全部 API 调用能力，但无一被 UI 调用。修复：① `MusicPageTab` 新增 `NOVIDROME = 5`；② MusicPage 新增 Navidrome 浏览 chip + 3 级层级浏览（艺术家→专辑→歌曲），每级有返回导航；③ 歌曲行支持「导入」按钮（单曲入库）+ 顶层「导入全部」按钮（整张专辑入库）；④ `MusicViewModel` 新增 `importNovidromeAlbum()` 批量导入方法；⑤ 切到 NOVIDROME tab 时自动 ping 连接（如未连接），失败时显示错误提示+重试按钮；⑥ 歌曲点击可直接流式试听（不导入）。
