...
55. **TodoRepository 显式事务移除 + @Monitor 异步保存 + WAL checkpoint** — 三轮修复，笔记保存可持久化但待办不行：① 移除 TodoRepository 全部 6 个写操作的显式事务（`beginTransaction`/`commit`/`rollBack`），改为与 NoteRepository 完全一致的 plain `await this.store.insert/update/delete()`——NoteRepository 无事务能持久化，显式事务反而可能引入 WAL 刷新时序问题；② `onSave` 从 fire-and-forget 改为 `.then()` 链式模式——`@Event` 要求 `=> void` 返回类型，sync 回调内调用 `handleTodoSave().then(() => { showAddSheet = false })` 既满足类型约束又确保 DB 写入完成后才关闭 sheet。③ 但杀后台后仍然消失——新增 `PRAGMA wal_checkpoint(FULL)` 在执行 insert 后强制 WAL 刷盘（catch 忽略不支持的实现）；④ `createTodo` 新增 post-insert 读回验证（`getById` 确认数据可读）；⑤ `.then()` 改为 `@Monitor('saveTrigger')` 模式——`onSave` sync 回调只设置 `pendingSave` 数据+递增 `saveTrigger`，`@Monitor` 触发真正的 async 保存，完成后 close sheet + 显式调用 `loadTodos()` 从 DB 重新加载确认持久化。
56. **Header 渐变过渡重构（无极模糊边界）** — 根因：TodosPage/PomodoroPage/SettingsPage 的 header 只有 inner title Row 有 `backgroundColor`，outer Column 无背景色但 backdropBlur 在 outer Column——导致梯度渐变从 16% 透明叠加开始且底色为透明，backdropBlur 的二元开/关边界在 header 底边形成可见切割线。修复：① 三个页面的 `backgroundColor` 从 inner title Row 移到 outer Column（匹配 NotesPage 验证过的模式），使整个 header 区域有统一 frosted 底色；② 梯度高度从 48vp 增至 72vp，延长模糊可见度淡出距离；③ 内容 spacer 相应从 120vp 增至 140vp。
